...
class AppointmentCreateView(AppointmentCreateMixin, CreateView):
    ...
    def get_form(self,form_class):
        ...
        form.fields['appointment_time'].widget = DateTimeWidget(options=dateTimeOptions, usel10n=True, bootstrap_version=3)
    ...
    def form_valid(self, form):
        ...
        end_date = form.cleaned_data['appointment_time'] + datetime.timedelta(hours=1)

        if not datetime.time(9,00) <= start_date.time() < datetime.time(18,00):
            form.add_error('appointment_time', 'Часы приема — 09:00-18:00')
            return self.form_invalid(form)
        print(Appointment.objects.filter(appointment_time__range=(start_date, end_date)))

        if Appointment.objects.filter(appointment_time__range=(start_date, end_date)):
